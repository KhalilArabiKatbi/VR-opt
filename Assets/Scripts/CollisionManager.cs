...
public class CollisionManager : MonoBehaviour
{
    ...
    private void ApplyContinuousContactForces(OctreeSpringFiller obj1, OctreeSpringFiller obj2, ContactPoint contact)
    {
        ...
        CollisionLayer layer1 = obj1.GetCollisionLayer();
        CollisionLayer layer2 = obj2.GetCollisionLayer();

        // Blend material properties
        float blendedYoungsModulus = (layer1.youngsModulus + layer2.youngsModulus) * 0.5f;
        float blendedDampingFactor = (layer1.dampingFactor + layer2.dampingFactor) * 0.5f;

        // Blend stiffness with restitution (softer for bouncier materials)
        float e = contact.restitution;
        float blendedStiffness = blendedYoungsModulus * (1f - e * 0.5f);  // Less stiff for high e
        ...
        float dampingMag = blendedDampingFactor * criticalDamping * Mathf.Max(-velocityAlongNormal, 0f);  // Damp only closing to avoid sticking
        ...
        if (contact.friction > 0.001f)
        {
            ...
            if (tangentialMag > 0.001f)
            {
                ...
                float staticFrictionCoefficient = contact.friction * 1.2f;
                float dynamicFrictionCoefficient = contact.friction;

                float frictionCoefficient = tangentialMag < 0.01f ? staticFrictionCoefficient : dynamicFrictionCoefficient;
                float viscousMag = frictionCoefficient * viscousFrictionFactor * normalForceMag; // Proportional to normal force
